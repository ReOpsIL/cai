...
mod tests {
    ...
    fn create_test_files(temp_dir: &TempDir) -> Vec<String> {
        ...
        File::create(&file_paths[0]).unwrap().write_all(b"Content of test1").unwrap();
        File::create(&file_paths[1]).unwrap().write_all(b"Content of test2").unwrap();
        File::create(&file_paths[2]).unwrap().write_all(b"Content of other file").unwrap();
    ...
    fn test_command_regex_patterns() {
        ...
        let command_result = commands_registry::parse_command("@read-files(memory-id, wildcard.txt)").unwrap();
