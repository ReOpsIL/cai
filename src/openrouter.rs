...
pub async fn list_openrouter_models(api_key: &str) -> Result<Vec<Model>, Box<dyn std::error::Error>> {
...
pub async fn call_openrouter_api(api_key: &str, prompt: &str) -> Result<String, Box<dyn std::error::Error>> {
    ...
    let response_content = json["choices"][0]["message"]["content"].as_str().unwrap_or("No response");
